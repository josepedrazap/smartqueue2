...
export default class CustomComponent extends Component {
    ...
    .then((data) => {
      if(data.length == 0){
        ...
        for(var i = 0; i < data.length; i++){
          c_lat += data[i].lat;
          c_long += data[i].long;
          aux[i] = {'key': i, 'position': [data[i].lat, data[i].long], 'children': data[i].name}
        }
        c_lat = c_lat / data.length;
        c_long = c_long / data.length;
  ...
  render() {
    const center = [this.state.center_lat, this.state.center_long]
    ...
    return (
      <Map className="map" center={center} zoom={this.state.zoom}>
        ...
        />
        <MyMarkersList markers={this.state.marks} />
      </Map>
    )
  }
}
